...
export class Quaternion {
  ...
  static DIRECTION_Z: Quaternion = new Quaternion()
   ...
   */
  constructor(q1: Quaternion, q2: Quaternion, interp: number);
  ...
  constructor(arg1?: Quaternion|number[]|number, arg2?: Quaternion|number, arg3?: number, w?: number) {
    if (arg1 instanceof Quaternion) {
      if (arg2 instanceof  Quaternion && isNumber(arg3)) {
        this.slerp(arg1, arg2, arg3);
      } else {
        this.x = arg1.x;
        this.y = arg1.y;
        this.z = arg1.z;
    ...
    if (isNumberArray(arg1)) {
      this.fromAngles(arg1)
    } else if (isNumber(arg2) && isNumber(arg2) && isNumber(arg3) && isNumber(w)) {
      this.x = arg1;
      this.y = arg2;
      this.z = z;
      this.w = w;
   ...
   * @return this
   */
  public setQuat(q: Quaternion): Quaternion {
    this.x = q.x;
    this.y = q.y;
    this.z = q.z;
    this.w = q.w;
   ...
   */
  public fromAnglesArray(angles: number[]): Quaternion {
    if (isNumberArray(angles) && angles.length !== 3) {
   ...
   *            rot around z)
   */
  public fromAngles(xAngle: number, yAngle: number, zAngle: number): Quaternion {
    let angle: number;
    let sinY: number;
    let sinZ: number;
    let sinX: number;
    ...
    const cosYXcosZ: number = cosY * cosZ;
    const sinYXsinZ: number = sinY * sinZ;
    const cosYXsinZ: number = cosY * sinZ;
    const sinYXcosZ: number = sinY * cosZ;
  ...
  public toAngles(angles?: number[]): number[] {
    if (!angles) {
      angles = [];
    } else if (isNumberArray(angles) && angles.length != 3) {
      throw new Error("Angles array must have three elements");
    }
    ...
    let unit: number = sqx + sqy + sqz + sqw; // if normalized is one, otherwise
    // is correction factor
    let test: number = this.x * this.y + this.z * this.w;
    if (test > 0.499 * unit) { // singularity at north pole
      angles[1] = 2 * Math.atan2(this.x, this.w);
      ...
      angles[1] = Math.atan2(2 * this.y * this.w - 2 * this.x * this.z, sqx - sqy - sqz + sqw); // yaw or heading
      angles[2] = Math.asin(2 * test / unit); // roll or bank
      angles[0] = Math.atan2(2 * this.x * this.w - 2 * this.y * this.z, -sqx + sqy - sqz + sqw); // pitch or attitude
    }
    return angles;
  }
